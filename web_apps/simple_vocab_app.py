...
async def browse_enhanced(
    ...
    try:
        # Use proper consolidated browsing
        ...
        # Group by term ONLY - not by domain
        sql += " GROUP BY d.term"
        ...
        if letter and len(letter) == 1:
            count_sql += " AND d.term LIKE %s"
            count_params.append(f"{letter}%")
        
        if domain:
            count_sql += " AND wd.primary_domain = %s"
            count_params.append(domain)
        
        if part_of_speech:
            count_sql += " AND d.part_of_speech = %s"
            count_params.append(part_of_speech)
        ...
        cursor.close()
        conn.close()
        return {"words": words, "total": total_count}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
        ...
        <body>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3">
                            ...
                            <select id="pos-filter" class="form-select">
                                ...
                                {chr(10).join([f'<option value="{pos}">{pos}</option>' for pos in parts_of_speech])}
                            </select>
                        </div>
                        <div class="col-md-3">
            ...
            <script>
                ...
                function updatePagination() {{
                    ...
                    resultsInfo.className = 'text-center mt-2 text-muted';
                    resultsInfo.textContent = `Showing ${{startResult}}-${{endResult}} of ${{totalResults}} results`;
                    
                    // Remove existing results info if present
                    const existingInfo = paginationSection.querySelector('.text-center.mt-2.text-muted');
                    if (existingInfo) existingInfo.remove();
                    
                    paginationSection.appendChild(resultsInfo);
                }}
